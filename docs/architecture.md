...
2. `validate_content.py`が許可フィールド、表紙順序、必須項目、件数、型、参照、未確定文言を検証する。
...
| schema validator | `content.json` | 許可フィールド、表紙順序、必須項目、件数、型、参照整合性の合否 | 見た目、内容の事実性 |
...
lead未指定時は従来の本文領域をそのまま返し、指定時だけ実測したleadの高さに応じて本文上端を下げる。
...
|---|---|---|---|
| schema検証 | `content.json` | 未知キー、表紙順序、必須項目不足、件数超過、不正な参照、未確定文言、廃止type | 見た目、意味・事実の正しさ |
