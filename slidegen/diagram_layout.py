...
MIN_CONTAINER_CLEARANCE = 0.28  # 前行ラベルと次行コンテナ上辺の必須間隔
...
class Layout:
    def __init__(self, spec, reserve_note=False, content_area=None):
        ...
        if content_area is None:
            ...
            x0, x1 = AREA[0], AREA[2]
            if self.dense:
                y0, y1 = content_area.top - 0.12, content_area.bottom + 0.19
            else:
    ...
    def _auto_rows(self):
        ...
        for i in range(1, len(self.rows)):
            ...
            # lead指定時は利用可能高さが減るが、前行ラベルと次行コンテナの
            # 見出し帯まで裁量余白として比例圧縮すると境界線がsubを横切る。
            # 最小クリアランスは必須分へ移し、残りの帯と通常行間だけを圧縮する。
            if self.strict_container_clearance:
                required_extra = min(extra, MIN_CONTAINER_CLEARANCE)
                base += required_extra
