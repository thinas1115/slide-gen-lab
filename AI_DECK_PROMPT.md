...
2. 必須内容を対応済みtypeへ割り当てる。表現に必要なtypeがない場合、既存typeへ無理に当てはめない。
   リポジトリを編集できるAIは`EXTENDING.md`に従ってrenderer一式を実装・検証してからJSONを作る。
   JSON作成だけを担当するAIは、不足している表現と必要な入力構造を報告して停止する。
...
2. `meta.title`, `slides[*].title`, `slides[*].kicker`, 本文はすべて「資料要件」の
   ...
   `slides[0]`は`type: "title"`とし、表紙はデッキ内に1枚だけ置く。
