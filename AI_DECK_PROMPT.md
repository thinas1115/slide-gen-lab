...
1. `CONTENT_SCHEMA.md` と同じJSON構造で出力する。資料要件にない任意フィールドは出力しない。
   特に `lead`, `note`, `actor`, `emphasis`, 表紙の日付・作成者・フッターは、必要性と値が明示された場合だけ書く。
2. `meta.title`, `slides[*].title`, `slides[*].kicker`, 本文はすべて「資料要件」の
   テーマ・想定読者・目的に合わせて新規作成する。
   表紙の`title`は資料の主題、`subtitle`は対象範囲または目的を1文で書く。ページ数、ファイル形式、
...
13. `process`は直線工程なら`steps`、分岐・合流・差戻しがあるなら`flow.nodes / flow.levels / flow.edges`を使う。担当者が不要なノードへ`actor`を捏造しない。
14. JSON以外の説明文を付けない。
