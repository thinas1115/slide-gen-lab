...
| ファイル | 役割 |
|---|---|
| `sysA_pptx/generate.py` | 基本renderer (title/bullets/cards/table/twocol/chart) + 共通ヘルパー + ページ定数 |
| `sysA_pptx/diagrams.py` | hub/org renderer + 図解部品 (icon_node/box_node/add_arrow/arrow_label/container) |
| `sysA_pptx/diagrams2.py` | process/roadmap/matrix renderer |
| `sysA_pptx/diagrams3.py` | route() 直角配線 |
| `sysA_pptx/diagram_layout.py` | 宣言的レイアウトエンジン (グリッド仕様→座標。diagram type の本体) |
| `sysA_pptx/diagram_specs.py` | サンプル図のグリッド仕様 (aws_simple/aws_multiaz)。仕様の書き方見本 |
| `sysA_pptx/validate_content.py` | content.json の生成前検証。typeごとの `_v_*` 関数 |
| `sysA_pptx/generate_from_json.py` | content.json→PPTX。**新規資料の正式経路** |
| `sysA_pptx/generate_patterns.py` + `content_patterns.py` | 全typeの検証ギャラリー |
| `sysA_pptx/check_layout.py` | 生成済みPPTXの重なり・はみ出し機械検知 |
| `sysA_pptx/textfit.py` | フォント実測 (游ゴシックをPillowで測る) |
| `sysA_pptx/fetch_fluent_icons.py` / `extract_aws_icons.py` | アイコン素材の追加取得 (assets/ は同梱済み。条件は assets/CREDITS.md。Fluentは要 svglib+reportlab+rlPyCairo) |
...
```powershell
python sysA_pptx/validate_content.py content.json                       # 新typeのschema検証
python sysA_pptx/generate_from_json.py content.json out\deck.pptx       # 新規資料経路
python sysA_pptx/generate_patterns.py out\pattern_gallery.pptx          # ギャラリー(全type)
python sysA_pptx\check_layout.py out\pattern_gallery.pptx               # exit 0 必須
