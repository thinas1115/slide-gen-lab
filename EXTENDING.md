...
| L1 レイアウト計算 | diagram_layout(グリッド図解) / timeline_layout(フェーズ・マルチトラック工程表) / image_slide(大判画像の比率維持・トリミング) / matrix(散布) / bullets・cards・twocol(縦詰め) / org(ツリー) / hub(放射) | **△ ジャンル内のみ** |
...
| `slidegen/image_slide.py` | 大判画像の比率維持・中央トリミング・caption/sourceを含む段階的収容 |
